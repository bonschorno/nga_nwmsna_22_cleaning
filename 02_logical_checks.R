...
# Manually implementing logical checks ---------------------------------------------

# total issues
issue_data %>% 
  nrow()

# see where most errors occurred
issue_data %>% 
  group_by(check_failed_num, check_failed, question.name) %>% 
  count()

# how many with several issues?
...
# start to end not shorter than 20mins
library(lubridate)
...
min(time$start_to_end)
median(time$start_to_end)

time %>% 
  filter(start_to_end < quantile(start_to_end, prob = 0.90)) %>% 
  ggplot() +
  geom_histogram(aes(x = start_to_end), bins = 100)

# check1: source of income

data %>% 
  filter(overall_income_estimate == "less_than" & source_of_income == "less_than")

# check2: source of income

data %>% 
  filter(overall_income_estimate != "less_than" & source_of_income == "none")

# check3: lack income cope 

data %>% 
  filter(overall_income_estimate == "less_than" & lack_income_cope == "none") %>% 
  pull("_uuid")

# check4: shelter enclosure issues

check4_data <- data %>% 
  filter(shelter_damages_defects != "no_damage" & shelter_enclosure_issues == "no_issues") %>% 
  select(shelter_damages_defects, shelter_enclosure_issues)

# check5: nearest health care

check5_data <- data %>% 
  filter((nearest_health_care_children=="no_facility_nearby" | nearest_health_care_adult=="no_facility_nearby") & nearest_health_care == "less_than_15") %>% 
  select(contains("nearest_health_care"))

# check6: diff_or_shocks
check6_data <- data %>% 
  filter(!diff_or_shocks %in% c("no_shocks", "do_not_know", "prefer_not_to_answer") & year_first_displacement_recent == 2021) %>% 
  select(diff_or_shocks, year_first_displacement_recent)

# check7: main_source_electricity

data %>% 
  filter(main_source_water == "bottle_water" & overall_income_estimate == "less_than") %>% 
  nrow()
